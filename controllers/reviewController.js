...
export function addReview(req, res) {
    ...
    newReview.save()
        .then(() => {
            res.json({ message: "Review was saved successfully" });
        }).catch((error) => {
...
export function deleteReview(req, res) {
    ...
    if (req.user.role == "admin") {
        ...
        }).catch(() => { res.status(500).json({ error: "Review deletion failed" }) })
...
export function approveReview(req, res) {
    ...
    if (req.user.role == "admin") {
        ...
        res.status(403).json({error : "Your are not an admin, Only the admins can approve reviews"});
    }
}